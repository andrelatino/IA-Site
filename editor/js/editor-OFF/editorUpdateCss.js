...
function addOrUpdateSCss() {
  const elementIDText = document.getElementById('elementID-text');
  const localStorageStyleID = localStorage.getItem('styleID');
  const styleID = localStorageStyleID;
  const styleValue = document.getElementById("s-textarea").value;
  const elementID = elementIDText.innerText;
  ...
  if (styleElement) {
    ...
    // Check if a rule for the current element ID already exists with the same media query
    const regex = new RegExp(`@media screen and \\(min-width:641px\\) and \\(max-width:768px\\){#${elementID}{[^}]*}}`, 'g');
    const matchedRules = existingCSS.match(regex);
    ...
    if (matchedRules) {
      if (styleValue.trim() === '') {
        ...
        const updatedCSS = existingCSS.replace(regex, '');
        // Remove the media query if there are no rules left
        const emptyMediaQueryRegex = /@media screen and \(min-width:641px\) and \(max-width:768px\) \{\s*\}/g;
        const updatedCSSWithoutMediaQuery = updatedCSS.replace(emptyMediaQueryRegex, '');
        styleElement.innerHTML = updatedCSSWithoutMediaQuery.trim();
      } else {
        // Replace the existing rule with the new rule
      ...
      // Add the new rule to the existing rules, only if the styleValue is not empty
      const mediaQuery = `@media screen and (min-width:641px) and (max-width:768px)`;
      const newCSSRuleWithMediaQuery = `${mediaQuery}{${newCSSRule}}`;
...
document.getElementById("s-textarea").addEventListener("input", addOrUpdateSCss);
...
function addOrUpdateMCss() {
  ...
  if (styleElement) {
    ...
    const newCSSRule = `#${elementID}{${formattedStyleValue}}`;
    // Check if a rule for the current element ID already exists with the same media query
    const regex = new RegExp(`@media screen and \\(min-width:769px\\) and \\(max-width:1024px\\){#${elementID}{[^}]*}}`, 'g');
    const matchedRules = existingCSS.match(regex);
    ...
    if (matchedRules) {
      if (styleValue.trim() === '') {
        ...
        const updatedCSS = existingCSS.replace(regex, '');
        // Remove the media query if there are no rules left
        const emptyMediaQueryRegex = /@media screen and \(min-width:769px\) and \(max-width:1024px\) \{\s*\}/g;
        const updatedCSSWithoutMediaQuery = updatedCSS.replace(emptyMediaQueryRegex, '');
        styleElement.innerHTML = updatedCSSWithoutMediaQuery.trim();
      } else {
        // Replace the existing rule with the new rule
      ...
      // Add the new rule to the existing rules, only if the styleValue is not empty
      const mediaQuery = `@media screen and (min-width:769px) and (max-width:1024px)`;
      const newCSSRuleWithMediaQuery = `${mediaQuery}{${newCSSRule}}`;
...
function addOrUpdateLCss() {
  const elementIDText = document.getElementById('elementID-text');
  const localStorageStyleID = localStorage.getItem('styleID');
  const styleID = localStorageStyleID;
  const styleValue = document.getElementById("l-textarea").value; // Changed ID to "l-textarea"
  const elementID = elementIDText.innerText;
  ...
  if (styleElement) {
    ...
    // Check if a rule for the current element ID already exists with the same media query
    const regex = new RegExp(`@media screen and \\(min-width:1025px\\) and \\(max-width:1280px\\){#${elementID}{[^}]*}}`, 'g');
    const matchedRules = existingCSS.match(regex);
    ...
    if (matchedRules) {
      if (styleValue.trim() === '') {
        ...
        const updatedCSS = existingCSS.replace(regex, '');
        // Remove the media query if there are no rules left
        const emptyMediaQueryRegex = /@media screen and \(min-width:1025px\) and \(max-width:1280px\) \{\s*\}/g;
        const updatedCSSWithoutMediaQuery = updatedCSS.replace(emptyMediaQueryRegex, '');
        styleElement.innerHTML = updatedCSSWithoutMediaQuery.trim();
      } else {
        // Replace the existing rule with the new rule
      ...
      // Add the new rule to the existing rules, only if the styleValue is not empty
      const mediaQuery = `@media screen and (min-width:1025px) and (max-width:1280px)`;
      const newCSSRuleWithMediaQuery = `${mediaQuery}{${newCSSRule}}`;
...
document.getElementById("l-textarea").addEventListener("input", addOrUpdateLCss); // Changed ID to "l-textarea"

function addOrUpdateXlCss() {
  const elementIDText = document.getElementById('elementID-text');
  const localStorageStyleID = localStorage.getItem('styleID');
  const styleID = localStorageStyleID;
  const styleValue = document.getElementById("xl-textarea").value;
  const elementID = elementIDText.innerText;
  ...
  if (styleElement) {
    ...
    // Check if a rule for the current element ID already exists with the same media query
    const regex = new RegExp(`@media screen and \\(min-width:1281px\\){#${elementID}{[^}]*}}`, 'g');
    const matchedRules = existingCSS.match(regex);
    ...
    if (matchedRules) {
      if (styleValue.trim() === '') {
        ...
        const updatedCSS = existingCSS.replace(regex, '');
        // Remove the media query if there are no rules left
        const emptyMediaQueryRegex = /@media screen and \(min-width:1281px\) \{\s*\}/g;
        const updatedCSSWithoutMediaQuery = updatedCSS.replace(emptyMediaQueryRegex, '');
        styleElement.innerHTML = updatedCSSWithoutMediaQuery.trim();
      } else {
        // Replace the existing rule with the new rule
      ...
      // Add the new rule to the existing rules, only if the styleValue is not empty
      const mediaQuery = `@media screen and (min-width:1281px)`;
      const newCSSRuleWithMediaQuery = `${mediaQuery}{${newCSSRule}}`;
...
document.getElementById("xl-textarea").addEventListener("input", addOrUpdateXlCss);
