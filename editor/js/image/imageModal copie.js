function sectionImage(button) {
    ...
    var loadAllImageUrl = image.getAttribute('src');

        // Check if the src attribute is empty or null
    if (!loadAllImageUrl) {
      // Set a new image source when the src is empty
      image.src = './assets/svg/icons/image.svg'; // Replace 'path/to/another/image.jpg' with the path to the image you want to display.
    }
    else{
      image.src = loadAllImageUrl;
    }

    alert(loadAllImageUrl);
    ...
    if (editorDiv) {
      ...
      <div id="image-container">
            ...
            <div id="image-github-buttons">
              ...
              <button id="image-btn-xs" onclick=" imageXsButton(); getXsImage(); updateImageXS();">XS</button>
              <button id="image-modal-search" onclick="openImageSidebar();"><img src="./assets/svg/icons/search.svg"></button>           
            </div>
            
            <div id="image-all">          
                <img id="image-all-thumbnail" src="${loadAllImageUrl}">           
                <input type="text" id="image-all-input" value="${loadAllImageUrl}">
